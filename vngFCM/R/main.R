...
preproc_facs_plates <- function(.dirs, .data2preproc,
  ...
  scattering_filter(.dirs, .data2preproc, .f_par, .filter_preproc_namer, .force, .preproc_script, .jobs_cmd_name, .verbose)
  ...
  for (.dir in .dirs) {
    ...
    .pls_l <- mapply(function(.x1, .x2) c(.x1, list(.x2)), .pls_l,
                     remove_noise(.dir, .preproc_dir, .f_par, .plot, .verbose, .pdf_dim, .cache_namer, .filter_preproc_namer, .drop_raw, .use_doParallel, .force),
                     SIMPLIFY = FALSE)
  }
